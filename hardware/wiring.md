...
| **ปุ่ม JUMP** | — | **D4** | ของใหม่ 2026-09-21 ขาว่าง ไม่ชนหน้าที่อะไร |
...
**ปุ่มกด (tact switch 4 ขา) — D4**
ต่อระหว่างขา D4 กับ GND ใช้ `INPUT_PULLUP` ภายใน **ไม่ต้องมี R ภายนอก** กดแล้วขาลง LOW
ขาทั้ง 4 ของ tact switch จับคู่เชื่อมกันอยู่ภายในทีละคู่ **ต้องใช้ขาทแยงมุม** ไม่งั้นจะเหมือนกดค้างตลอดเวลา

ปุ่มเดียวทำ 2 หน้าที่ ตัดสินจากสถานะตอนกด:
- **โครงนอนพิงอยู่** → สั่งลุกขึ้นเอง หาฝั่งเองจากเครื่องหมายของ θ (`side_ = thetaDeg > 0 ? 1 : -1`) ใช้ได้ทั้ง +θ และ −θ โดยไม่ต้องบอกฝั่ง
- **กำลังทรงตัว/กำลังลุก** → หยุดทันที เทียบเท่าคำสั่ง `stop` เป็นทางเดียวที่หยุดเครื่องได้เมื่อไม่ได้เสียบ USB

เงื่อนไขความปลอดภัยใช้ชุดเดียวกับคำสั่ง `jump` ทุกข้อ (ฟังก์ชัน `startJump()` ตัวเดียว ทั้งปุ่มและ serial เรียกร่วมกัน เพื่อไม่ให้เงื่อนไขสองทางหลุดจากกัน)
ปิดการใช้งานได้ด้วย `ENABLE_JUMP_BUTTON 0` ใน `config.h` ถ้ายังไม่ได้ใส่ปุ่ม
