...
## 🎉 Jump-up สำเร็จ — ค่าท่ีทำงานได้ (2026-09-20, ~00:44)
- **`jump 550 450 8`** = ค่ากลางท่ีทำงานได้: ลุกจาก -15.5° ขึ้นมาตั้งตรง แล้วทรงตัวต่อเองใน ~1.4 s (log: scratchpad `jump_success2.log` line 3146+, 0 FALLEN)
  - เปรียบเทียบ: `500/350/10` = ไม่ถึง (ค้าง -4.5°, เร็วสุด 50°/s) · `550/450/6` = แรงเกิน (พุ่ง 200°/s, ส่งต่อที่ -5.3° เร็วเกิน, ล้ม +15.9°)
- ⚠️ **ยังต้องทดสอบซ้ำ 2-3 ครั้งเพื่อความชัวร์** ก่อนตั้งเป็นค่าเริ่มต้นถาวร
...
## อัปเดต 2026-09-20 (ต่อ) — คำสั่ง jump 3 พารามิเตอร์ + แหล่งไฟ
- **คำสั่ง `jump` รับ 3 พารามิเตอร์แล้ว** (ไม่ต้องแฟลชระหว่างเทส): `jump [rpm] [kick_ms] [capture_deg]` (capture 3-15°) · `jumpcfg` พิมพ์ค่า · compile ผ่าน, selftest ผ่าน 3 ข้อ
- **แหล่งจ่ายไฟ 12V เปลี่ยน/ปรับไม่ได้เลย** (ผู้ใช้บอก) → แก้ที่ซอฟต์แวร์: หมุนล้อก่อนกลับทิศที่ 550 rpm (โมเมนตัมมากขึ้น) + ปรับมุมส่งต่อ (capture_deg)

## ลุกขึ้นเองแบบคงที่ (2026-09-21)
- **กติกาส่งต่อใหม่: คุมความเร็วแทนมุม** — ตัวชี้วัดที่ตัดสินผลคืออัตราเอียงตอนส่งต่อ (ดู `tuning/jump_runs.csv`)
  เตะจนอัตราเอียง > 90 °/s → **COAST** (ปล่อยไหล) → ส่งต่อเมื่อ |θ| ≤ 8° และอัตราเอียงอยู่ใน 60–90 °/s · ถ้าไหลช้ากว่า 60 °/s กลับไปเตะต่อ
- `jump 550 450 8 75` **สำเร็จ 3/3** (นิ่งใน 1.0–2.6 s, เลยไปอีกฝั่ง 5.7–7.4°) เทียบกับแบบมุมคงที่ที่ได้ 3/10 · ตั้งเป็นค่าเริ่มต้นใน `config.h` แล้ว
- ลองสูตรพลังงาน `v ≥ √(2K(1−cosθ))` แล้ว **ใช้ไม่ได้** (K=70 ทั้งแบบไม่จำกัดมุมและจำกัด ≤10° ยังเลยไปล้ม) เพราะหลังส่งต่อโหมดทรงตัวออกแรงเพิ่มด้วย
- คำสั่ง: `jump [rpm] [kick_ms] [capture_deg] [rate_dps]` · `jumpcfg` แสดงค่าปัจจุบัน
- **ข้อมูลการจูนทั้งหมดย้ายไปอยู่ใน `tuning/`** (`jump_runs.csv`, `balance_gains.md`, `motor_fg.md`, `logs/*.log.gz`, `tools/`)

## Session 2026-09-22 → 23 (Opus/Fable) — ฐานลื่นคือตัวแปรหลัก, ลิงก์ serial คือตัวขวาง

**อ่านก่อนทำต่อ:** `tuning/README.md` หัวข้อ 2026-09-22/23 · ข้อมูลสะอาดชุดเดียวคือ `tuning/nano_2026-09-22_left_after_base_fix.csv` (33 ครั้ง, 7 สำเร็จ)

### สิ่งที่พิสูจน์แล้ว (เชื่อได้)
- **ฐานลื่น = ตัวแปรที่ทำให้ 90 ครั้งก่อนหน้าจูนไม่ขึ้น** ก่อนแก้: ความเร็วส่งต่อ SD 21.5 dps, corr(เวลา, ความเร็วส่งต่อ) = −0.79, มุมนอนไหล +0.56 ตามเวลา · หลังใส่ยาง/ตัวกั้น: SD 9.6, corr −0.28, มุมนอนนิ่ง (+0.06) · **ต่อจากนี้เทียบ A/B ต้องสลับ A,B,A,B เท่านั้น** (`autofire.py --params-b`)
- **หน้าต่างส่งต่อ 53–61 dps**: 7 ครั้งที่สำเร็จอยู่ในนี้ทั้งหมด · >78 dps เลยไปอีกฝั่ง 16–19° (0/9) · <46 dps ขึ้นไม่ถึง duty ตัน 27–56% (0/12) · ในหน้าต่าง ล้อ ≥130 rpm → 7/8 · **ยังไม่ได้ทดสอบกับข้อมูลใหม่ ห้ามใส่ config จนกว่าจะยืนยัน**
- ตัวคั่นผลที่ทำนายได้จริงตัวเดียว: duty ตันใน 1 วิแรกหลังส่งต่อ (สำเร็จ 0–12%, ล้ม 14–56%) — `jumpruns.py` คำนวณให้
- **spin-up คงที่ทั้งวัน** (rpm@300ms 440–449 ทุก session, `tuning/tools/spinup.py`) → ไฟ 12V ไม่ตก ตัดเรื่องแบตทิ้งได้
- COAST สั้นแค่ 0–80 ms ในทุกครั้ง → `JUMP_COAST_HOLD_DUTY` (0.20 ใน config, arg ที่ 5 ของ `jump`) **น่าจะมีผลน้อยกว่าที่ออกแบบไว้** ยังไม่มีข้อมูลสะอาดตัดสิน (ชุด A/B ทั้งสองถูกลิงก์เพี้ยนปน ทิ้งหมด)
- **sim 2 ตัว (Codex `sim/capture_set.py`, Fable `sim/nano_replica.py`) ไม่ผ่านด่านทำนาย 25 ครั้งที่รู้ผล** ตัวแรกทายล้มหมด ตัวหลังทายรอดหมด — ห้ามใช้แผนที่ของทั้งคู่ · ของมีค่าจาก Fable: มอเตอร์เร่งจากนิ่งได้ ~200 rad/s² (ไม่ใช่ 40 — ตัวนั้นคือ back-EMF ที่ 350–450 rpm), friction offset 0.035 duty, accel lever-arm ทำมุมที่ log กระโดด +4° ตอนส่งต่อ, SD มุมจริง (gyro) 0.15° ไม่ใช่ 0.27

### ปัญหาที่ยังค้าง — ลิงก์ serial ผ่าน CH340 บนบอร์ด
- อาการ: **บอร์ดส่งออกได้ แต่รับคำสั่งไม่ได้** (`get` 0/10, avrdude "not in sync", DTR reset ไม่ถึงชิป) · เกิด 4 ครั้ง ทุกครั้งหลังยิงมอเตอร์หลายครั้งหรือหลังสับสวิตช์ 12V · หายเอง 2 ครั้ง (ไม่ใช่เพราะเสียบใหม่อย่างที่คิดตอนแรก) · ครั้งสุดท้ายไม่หาย: ลอง 3 พอร์ต 2 สาย ถอดสายไดรเวอร์ทั้ง 4 เส้นแล้ว
- **Blink 1 KB ก็อัปโหลดไม่ได้** (not in sync resp=0x00 เหมือนกัน) → ยืนยันว่าเป็นลิงก์ CH340→ATmega ไม่ใช่สเก็ตช์/ขนาดแฟลช
- แฟลชล้มเหลวกลางคันทำสเก็ตช์เสียได้ (เกิดแล้ว 1 ครั้ง กู้ด้วยแฟลชซ้ำ) · `pnputil /restart-device` แก้ได้ถ้ารันเทอร์มินัลแบบ **admin** (คราวนี้ Access denied)
- **ทางแก้ session หน้า (เลือกหนึ่ง):** USB-TTL แยก → D0/D1/RST(ผ่าน 100nF) · หรือ Mega เดิมรัน passthrough Serial↔Serial1 · หรือ Uno ที่ดึง RESET ลง GND เป็นสะพาน — ทั้งหมดข้าม CH340 บนบอร์ด · **ระยะยาว:** USB isolator หรือแยกกราวด์ USB จากกราวด์มอเตอร์
- เฟิร์มแวร์ที่**ยังไม่ได้ขึ้นบอร์ด** (compile ผ่าน, host test ผ่าน): ack ของ `jump` ทวนค่าที่รับ (`rpm= kick= cap= rate= hold=`) และ `autofire.py` ตรวจ echo · บนบอร์ดตอนนี้คือ commit `22510ee` (มี coast-hold แต่ไม่มี echo)

### กฎการทำงานที่ได้จากวันนี้
- **ทุกครั้งที่ RX ตาย ให้ทดสอบด้วยการแฟลช ไม่ใช่ `get`** — ถ้าสเก็ตช์เสียอยู่ `get` จะเงียบแม้ RX ดี
- **อย่าเชื่อผลใด ๆ ที่บอร์ดไม่ได้ตอบ `jump: SPINUP`** (`autofire.py` แก้ให้ปฏิเสธแล้ว 2 รอบ — บั๊กแรกเช็ก FALLEN ก่อน ack)
- ตั้งกฎจากข้อมูล ≥20 ครั้ง และต้องผ่านการทำนายล่วงหน้ากับชุดใหม่ (วันนี้คำทำนาย "rate 75 จะดี" ผิด 0/9 — จับได้เพราะประกาศก่อนยิง)
- Arduino Cloud Agent แย่งพอร์ตตอนแฟลช → ปิดก่อน · COM port เปลี่ยนทุกครั้งที่ย้ายพอร์ต (วันนี้ 9→7→9→8→7) ให้ detect ทุกครั้ง
